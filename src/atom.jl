...
mutable struct Atom
    ...
    #if <0, the atom is in free space, outside of a topological mode, at location r with velocity v
    edge_idx::Integer
    is_spin_up::Bool
    ...
    function Atom(;
        ...
        new(edge_idx, is_spin_up, Observable(r), v, tail_length, tail)
    end
end


function step!(a::Atom, l::Lattice; clip_distance::Real=0.5)
    #if not in edge state, atom moves with velocity v
    if a.edge_idx < 0
        x, y, z = a.r[]
