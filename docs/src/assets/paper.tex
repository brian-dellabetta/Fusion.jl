...
\title{``Ultracold Fusion''? Revisiting Colliding Beam Fusion in Topological Ultracold Atom Systems}

\author{Brian Dellabetta}
\email{brian.dellabetta@gmail.com}
\affiliation{https://www.linkedin.com/in/brian-dellabetta}


\date{\today}
%
\begin{abstract}
While thermonuclear fusion reactors are the established leading path to sustainable fusion power, commercial viability is impeded by the extreme temperatures and pressures they require. Fusion power concepts based on the simpler collision of individual pairs of atoms offer an appealing alternative. Collision-based schemes and applications appear in the literature as far back as the 1970s, but progress stalled because of fundamental issues largely stemming from the dispersion relation of massive atoms in free space -- significant energy is expended to accelerate atoms to a critical velocity, which in turn decreases the de Broglie wavelength and probability of quantum tunneling through the Coulomb barrier. Since those early investigations, significant progress in topological phases of matter and ultracold atom technology has shown that carefully designed optical lattices can yield atomic transport modes following a qualitatively different \textit{massless} dispersion relation along certain topological phase transitions -- high velocity and long wavelength at low energy. Moreover, these modes are confined to one dimension, cannot decelerate or backscatter, and are energetically separated from bulk scattering states. In this perspective, we propose that this emerging technology is the ideal environment to revive the study of practical nuclear fusion reactors based on the collision of individual pairs of atoms, with the promise of being more modular and sustainable than any of the leading thermonuclear candidates.

% \keywords{Colliding Beam \and Fusion \and Condensed Matter \and 1-D Systems \and Topological States \and Ultracold Atoms.}
...
Nuclear fusion reactors have been a topic of immense interest for decades -- a viable nuclear fusion reactor could provide immense amounts of clean energy to help resolve long-standing concerns of climate change driven by fossil fuels and ever-increasing global energy demand.

Virtually every current initiative to achieve practical nuclear fusion is thermodynamic in nature (e.g. magnetic and inertial confinement fusion \cite{Huang2018-po,RevModPhys.95.025005}). In thermodynamic nuclear fusion, a bulk fuel source is confined to an environment of immense temperature and pressure to force atomic nuclei close enough to overcome Coulomb repulsion, fuse into heavier nuclei and release energy. Creating an environment akin to an ``artificial sun'' is an extremely difficult engineering challenge, and it remains an open question whether a net positive energy can ever sustainably be harvested from a thermodynamic fusion reactor.

Fortunately, fusion is inherently a microscopic reaction between two atomic nuclei, and an environment of immense temperature and pressure is not mandatory for it to occur. Colliding Beam Fusion (CBF) is one such alternative, dating back at least to the 1970s. In CBF, two beams of atomic nuclei are directed at one another. Ideally, one atom in the first beam collides with another from the second beam at high enough velocity to fuse \cite{MAGLICH1973213,MAGLICH1975IEEE,blewett197xcbf}. 

The most naive CBF design is shown in Fig. \ref{subfig:CBF}. While CBF appears in the literature spanning multiple decades, it failed to ever produce a net positive energy (see Appendix \ref{sec:CBF} for a brief review). The fatal flaw ultimately lies in the parabolic dispersion relation of massive atoms in free space (see Table \ref{tab:dispersion}). First and foremost, significant energy ($\sim100$ keV per atom pair) is needed to accelerate atoms to a sufficient velocity, meaning CBF has a high variable energy cost. For massive particles, high energy will result in a short de Broglie wavelength ($E \propto 1/\lambda$), which will decrease the so-called ``geometric factor'' of the fusion cross section, $\sigma \propto \lambda_{dB}^2$ \cite{LI20061517}. Furthermore, the energy lost to screening and scattering effects into a continuum of lower-energy states, known as bremsstrahlung loss, is prohibitively large to ever yield a net positive energy \cite{Ridder1994}. For these reasons, CBF research appears to have stalled in the 1990s.
...
In free space, energy is required to accelerate a particle to a high velocity, and will yield a point-like particle with short wavelength. Qualitatively different dispersion is necessary for CBF to be viable -- high speed, low energy, and long wavelength (i.e. nuclei are more wave-like than point-like, so that their wave functions overlap). Condensed matter physics has a rich history demonstrating how a periodic potential can alter the behavior of charged particles relative to free space. 

More recently, topological phases in condensed matter have shown the desired dispersion relation can be realized along the boundary of certain topological phase transition (see Appendix \ref{sec:CondMat} for a brief review of topological phases in condensed matter). Namely, a signature of the Quantum Spin Hall (QSH) topological phase is a pair of massless, counterpropagating, one-dimensional modes along the phase transition.
...
\begin{tabular}{|M||M|M|}
 ...
 (Appendix \ref{sec:semiclassicalapproximation}) & exponential decay & \\
 \hline
...
The proposed enhancement to CBF is depicted in Fig. \ref{subfig:UCF} -- cool and place atomic nuclei in the counter-propagating, high-speed, high-wavelength modes that appear along the boundary of an optical lattice in a QSH topological phase. Given the long history of the term ``cold fusion'' in popular science, and the fact that this proposal argues for the novel use of ultracold atom technology for fusion, ``Ultracold Fusion'' seems an appropriate term to coin for the proposed concept.

To enumerate, the key enhancements in ``Ultracold Fusion'' are the following:
...
\begin{enumerate}
  ...
  \item Atoms in free space have a continuum of scattering states in which to decelerate or change trajectory. Ultracold atoms in topological edge modes are \textit{protected from backscattering} and are energetically separated from bulk bands \cite{RevModPhys.83.1057}. The loss of bremsstrahlung should effectively be zero in the ideal band structure shown in Fig. \ref{subfig:UCF}, and while scattering in bulk states is still possible, at least no energy will be lost to it. 
...
The design of transport modes most suitable for fusion of ultracold atomic nuclei offers a promising playground to revisit CBF, and potentially circumvent the extreme environments hampering sustainable thermodynamic fusion reactors. Optical lattices provide flexible control over the salient features for fusion -- effective mass, edge mode velocity, spatial confinement in the transverse directions, and band gap energy separation from bulk states. 

Ultimately, the viability of ultracold fusion will depend on estimating fusion and scattering rates as functions of edge mode velocity and bulk band gap separation, beyond the semiclassical picture (see Appendix \ref{sec:scattering} for an initial attempt). Achieving the same velocity used in CBF would require optical lattices with sub-nm laser wavelengths, approaching the limits of feasibility (see Appendix \ref{sec:critvel} for more detail). Would an optical lattice built with Extreme Ultraviolet (EUV) lasers, much more commonly used in photolithography \cite{Kazazis2024-is}, be able to trigger fusion with a velocity 3-4 orders of magnitude smaller? This calculation is crucial to determine viability, and remains an open question. Here, we simply hope to have motivated a promising general framework meriting further exploration, concluding with some open questions:
...
\item \textit{Would fusion cross section also be enhanced by a more favorable coupling constant at low/zero energy?} The fusion of two atomic nuclei is a tug-of-war between the attractive strong nuclear and the repulsive electromagnetic force. The strong force gets much stronger at lower energies, whereas the electromagnatic force gets slightly weaker at lower energies (see Ref. \cite{couplingcontantrunning} Fig. 6). If CBF operates at roughly 100 keV, and the proposed ultracold fusion can be performed effectively at zero energy, it seems reasonable to think this would further enhance fusion cross section. Moreover, the running of the strong nuclear force's coupling constant does not appear to be well-defined at low energy, so perhaps this provides a way to probe another interesting physical question.
\item \textit{What happens if fusion does occur?} The resultant high-energy (14 MeV) neutron as a charge-free particle would not be bound to the edge mode. Its energy could presumably be captured as heat in a heavy water reservoir. The resultant charged particle would have energy on the order of MeV, far exceeding the band gap, and could be captured easily. Could it inadvertently damage the optical lattice or topological phase? 
